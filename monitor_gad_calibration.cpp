...
int main(){
	...
	myplotter.purerefset="18mar2023";
	...
	std::string concentrationsfile="mar182023_calibration_info.txt";
...
int Plotter::LoadNewData(){
	...
	const char* dirname = "../GDConcMeasure/data/2023/03";
	...
	if(filestlist){
		...
		while( file = (TSystemFile*)next() ) {
			...
			TString expectedprefix="00150_18Mar2023Calibration";
...
TMultiGraph* Plotter::Execute(){
	...
	mg_cals->GetHistogram()->GetXaxis()->SetTitle("concentration [%Gd]");
	mg_cals->GetHistogram()->GetYaxis()->SetTitle("Peak height diff [no units]");
	//*/
	
	// to monitor stability after calibration:
	// take the results printed out by the above and populate the coefficients list
	// in ExtractAbsorbance, and re-analyse data to extract plots of stability
	...
	*/
	...
	std::cout<<"returning calibration data so far"<<std::endl;
	return mg_cals;
}
...
TMultiGraph* Plotter::FitCalibrationData(std::string name){
	...
	TChain* c_led = chains.at(name);
	TChain* c_dark = chains.at("dark");
	int n_entries = c_led->GetEntries();  // FINDME
	// FIXME hard-code to max npoints for follow-on stability measurements
	...
	for(int i=0; i<n_entries; i++){
		...
		// fit with pure scaled
		g_sideband->Fit(pureplusextras,"RNMQ"); // initial fit
		/*
		// this isn't sufficient as it would require us to retrieve the absorbance
		// from heights of the gaussians, not from the sum of gaussians;
		// we probably just compensate with another...
		pureplusextras->FixParameter(9,0.71);   // Fix the ratio of peak heights
		g_sideband->Fit(pureplusextras,"RNMQ"); // re-do fit
		pureplusextras->ReleaseParameter(9);    // this clears any limits
		pureplusextras->SetParLimits(9,0.,1.);  // re-set them
		*/
